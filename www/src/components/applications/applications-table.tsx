...
import { ResponseApplicationData, SafeUserData } from "@/lib/validation";
import {
    ColumnDef,
    ColumnFiltersState,
    flexRender,
...
const columns: ColumnDef<TableApplication>[] = [
    ...
    },
    {
        ...
        header: "Review",
        cell: ({ row }) => {
            const applicantionId = row.original.id;

            return (
                <Link type="link" href={`/applications/${applicantionId}`}>
...
export function ApplicationsTable({ user }: PageProps) {
